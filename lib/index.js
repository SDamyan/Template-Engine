...
/*
//taking the user input and adding it to answers
async function populate() {
  var answers = await init ();
  console.log(answers);
...
function getManagerHTML (answers) {
    ...
    <div class="card-body">
        <h5 class="card-title managerName">${answers.managerName}</h5>
      <h5 class="card-title managerTitle">
        <i class="fas fa-tasks">Manager</i></h5>
      <ul class="list-group list-group-flush">
          <li class="list-group-item managerID">${answers.managerID}</li>
          <li class="list-group-item managerEmail">${answers.managerEmail}</li>
          <li class="list-group-item officeNumber">${answers.officeNumber}</li>
      </ul> 
    </div>
  </div>`
